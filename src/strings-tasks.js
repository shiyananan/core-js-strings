...
function removeFirstOccurrences(str, value) {
  ...
  const sliceStart = str.indexOf(value);
  const sliceEnd = sliceStart + value.length;
  return str.substring(0, sliceStart) + str.substring(sliceEnd);
}
...
function removeLastOccurrences(str, value) {
  ...
  const sliceStart = str.lastIndexOf(value);
  const sliceEnd = sliceStart + value.length;
  return str.substring(0, sliceStart) + str.substring(sliceEnd);
}
...
function isPalindrome(str) {
  let revStr = str.toLowerCase().match(/[a-z]/g);
  revStr.reverse();
  revStr = revStr.join('');
  if (str.toLowerCase().match(/[a-z]/g).join('') === revStr) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentSplit.length; i += 1) {
    if (sentSplit[i].length > longestLength) {
      longestLength = sentSplit[i].length;
      longestWord = sentSplit[i];
    }
  }
  return longestWord;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < strSplit.length; i += 1) {
    let newWord = strSplit[i].split('');
    newWord = newWord.reverse().join('');
    revArray.push(newWord);
  }
...
function invertCase(str) {
  ...
  const invSent = [];
  for (let i = 0; i < strSplit.length; i += 1) {
    const wordSplit = strSplit[i].split('');
    const revCase = [];
    for (let j = 0; j < wordSplit.length; j += 1) {
      if (wordSplit[j].toUpperCase() === wordSplit[j]) {
        revCase.push(wordSplit[j].toLowerCase());
      } else {
        revCase.push(wordSplit[j].toUpperCase());
      }
    }
    invSent.push(revCase.join(''));
  }
  return invSent.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (
      ...
      encodedLetter = String.fromCharCode(str.charCodeAt(i) + 13);
    } else if (
